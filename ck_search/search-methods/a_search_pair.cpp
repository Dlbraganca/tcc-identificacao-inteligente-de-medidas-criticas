#include"./headers/a_search_pair.h"


	a_search_pair::a_search_pair() {};
	
	a_search_pair::a_search_pair(const char* CK_PARAM_FILE) : ck_search(CK_PARAM_FILE)
	{	
		search();
		report();
	};
	...
	//systemState a_search_pair::get_result() {
	//	return result;
	//}

	bool comparar_heuristicas_pair(systemState& a, systemState& b) {
		return a.get_heuristic() < b.get_heuristic(); //crescente
	};

	void sort_vector_pair(std::vector<systemState>& x) {
		std::sort(x.begin(), x.end(), &comparar_heuristicas_pair);
	}

	void a_search_pair::check_pair(std::vector<unsigned int>& x) {
		...
		for (size_t i = 0; i < x.size(); i++)
		{
			if (x[i] == 0)
			{
				...
				{
					unsigned int positionPair;
					if (pair[j][1] != 0 && pair[j][0] == position)
					{
						positionPair = pair[j][1] - 1;
						x[positionPair] = 0;
						break;
					}
					else if (pair[j][0] != 0 && pair[j][1] == position)
					{
						positionPair = pair[j][0] - 1;
						x[positionPair] = 0;
						break;
					}
				}
	...
	std::string a_search_pair::hash_key(std::vector<unsigned int> x) {
		std::string result;
		for (size_t i = 0; i < x.size(); i++)
		{
