function []=ck_search_cpp_12jun(sis_file, med_file,crit_med)
% Find critical ck-tuple through search algorithms
%clear
original_path=pwd; 
cd('..') % change path to \root folder
current_path=pwd; % get current path
%addpath(strcat(current_path,'\res'),path); % add resources folder
%addpath(strcat(current_path,'\bin'),path); % add binaries folder
%addpath(strcat(current_path,'\lib'),path); % add libraries folder
%addpath(strcat(current_path,'..\..\'),path); % add libraries folder
Hfile=strcat(strcat(original_path,'\Criticality Analisys\src\'),'H.txt');
Mfile=strcat(strcat(original_path,'\Criticality Analisys\src\'),'M.txt');
MUfile=strcat(strcat(original_path,'\Criticality Analisys\src\'),'MU.txt');
Afile=strcat(strcat(original_path,'\Criticality Analisys\src\'),'A.txt');
% Hfile='H.txt';
% Mfile='M.txt';
% MUfile='MU.txt';
% Afile='A.txt';
ck_setup_file='ck_directives.txt';
ck_file='critical_tuples.txt';
ck_setup_file=strcat(strcat(original_path,'\Criticality Analisys\src\'),'ck_directives.txt');
ck_file=strcat(strcat(original_path,'\Criticality Analisys\src\'),'critical_tuples.txt');


ck_cpp_exe=strcat(strcat(current_path,'\bin\'),'single_criticality.exe');
cd(original_path);
clc
...
fprintf(fid,'%d\n',dim);
fprintf(fid,'%s\n',criticality_type);
for i = 1 : length(crit_med)
    fprintf(fid,'%d ',crit_med(i));
end
fclose(fid);
...
switch criticality_type
    case {'munit','measurement','top'}
        ...
        %create tuple distribution plot
        ...
        xlswrite(xls_file_name,distribution,'distribution');
        fprintf('done !!!');
end
